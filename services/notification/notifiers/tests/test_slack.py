...
class TestSlackNotifier(object):
    ...
    def test_build_payload_without_attachments(self, dbsession, mock_configuration, sample_comparison):
        ...
        text = f"Coverage for <test.example.br/gh/{repository.slug}/commit/{commit.commitid}|{repository.slug}> *increased* `<test.example.br/gh/{repository.slug}/compare/{base_commit.commitid}...{commit.commitid}|+60.00%>` on `new_branch` is `60.00000%` via `<test.example.br/gh/{repository.slug}/commit/{commit.commitid}|{commit.commitid[:7]}>`"
        expected_result = {
            'attachments': [],
    ...
    def test_build_payload_with_attachments(self, dbsession, mock_configuration, sample_comparison):
        mock_configuration.params['setup']['codecov_url'] = 'test.example.br'
        comparison = sample_comparison
        notifier = SlackNotifier(
            repository=sample_comparison.head.commit.repository,
    ...
    def test_build_payload_with_message(self, dbsession, mock_configuration, sample_comparison):
        mock_configuration.params['setup']['codecov_url'] = 'test.example.br'
        comparison = sample_comparison
        notifier = SlackNotifier(
            repository=sample_comparison.head.commit.repository,

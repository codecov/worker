...
class TestCommentNotifier(object):
    ...
    @pytest.mark.asyncio
    async def test_build_message_negative_change(self, dbsession, mock_configuration, mock_repo_provider, sample_comparison_negative_change):
        mock_configuration.params['setup']['codecov_url'] = 'test.example.br'
        ...
        expected_result = [
            ...
            f"| [Impacted Files](test.example.br/gh/{repository.slug}/pull/{pull.pullid}?src=pr&el=tree) | Coverage Δ | Complexity Δ | |",
            f"|---|---|---|---|",
            f"| [file\\_1.go](test.example.br/gh/{repository.slug}/pull/{pull.pullid}/diff?src=pr&el=tree#diff-ZmlsZV8xLmdv) | `50.00% <0.00%> (-12.50%)` | `11.00% <0.00%> (+1.00%)` | :arrow_down: |",
            f"| [file\\_2.py](test.example.br/gh/{repository.slug}/pull/{pull.pullid}/diff?src=pr&el=tree#diff-ZmlsZV8yLnB5) | `50.00% <0.00%> (ø)` | `0.00% <0.00%> (ø%)` | |",
            f'',
            f'------',
            f'',
    ...
    @pytest.mark.asyncio
    async def test_send_actual_notification_new_no_permissions_post(self, dbsession, mock_configuration, mock_repo_provider, sample_comparison):
        notifier = CommentNotifier(
            repository=sample_comparison.head.commit.repository,
            title='title',
            notifier_yaml_settings={
    ...
    @pytest.mark.asyncio
    async def test_notify_unable_to_fetch_info(self, dbsession, mocker, sample_comparison):
        mocked_build_message = mocker.patch.object(CommentNotifier, 'build_message', return_value=Future())
        mocked_build_message.return_value.set_exception(TorngitClientError('code', 'response', 'message'))
        notifier = CommentNotifier(
            repository=sample_comparison.head.commit.repository,
            title='title',
            notifier_yaml_settings={

        ...
        if branch.attrib['vc'] == '0':
            ...
            if 'sl' in attribs:
                if attribs.get('offsetend') is not None:
                    branches[int(attribs['sl'])].append(('%(offset)s:%(offsetend)s' % attribs))
                else:
        ...
        if fileref is not None:
            ...
            if _file is not None:
                ...
                for _type, node in zip(repeat(None), method.getiterator('SequencePoint')):
                    ...
                    if sl and el:
                        complexity = int(attrib('cyclomaticComplexity', 0)) if _type == 'm' else None
                        sl, el = int(sl), int(el)
                            ...
                            for ln in range(sl, el+1):
                                file_append(ln,
                                            ReportLine(coverage=coverage,
                                                       type=_type,
                                                       sessions=[[sessionid, coverage, branches_get(ln), None, complexity]],
                        ...
                        # spans = 1 line
                            ...
                            file_append(sl,
                                        ReportLine(coverage=coverage,
                                                   type=_type,
                                                   sessions=[[sessionid, coverage, branches_get(sl), None, complexity]],
                                                   complexity=complexity))

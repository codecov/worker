    ...
    for filename, data in report_dict.items():
            ...
            if branch.get('skip') is not True:
                if branch.get('type') == 'if':
                    ...
                    for lid, location in enumerate(branch['locations']):
                        ln, cov, partials = get_line_coverage(location, data['b'][bid][lid], 'b')
                        if ln:
                    ...
                    for ln, partials in line_parts.items():
                        ...
                        if len(partials) > 1:
                            ...
                            cov = line_cov[ln][0]
                            partials = None
                        _file.append(ln, ReportLine(cov, 'b', [[sessionid, cov, branches, partials]]))
                    ...
                    if line:
                        ...
                        if inline_part:
                            ...
                            if sc > ec:
                                ...
                                _file[sl] = ReportLine(cov, 'b', [[sessionid, cov, mb, cur_partials]])
                            else:
                                partials = [[sc, ec, cov]]
                                found = False
                                ...
                                _file[sl] = ReportLine(cov, 'b', [[sessionid, cov, mb, sorted(partials, key=lambda p: p[0])]])
        ...
        for fid, func in must_be_dict(data['fnMap']).items():
            if func.get('skip') is not True:
                ...
                if ln:
                    _file.append(ln, ReportLine(cov, 'm', [[sessionid, cov, None, None]]))
...
def _location_to_lines(_file, location, cov, _type, sessionid):
    ...
    _file.append(int(location['start']['line']), ReportLine(cov, _type, [[sessionid, cov]], None))

...
class TestCommitStatus(object):
    ...
    def test_ci_providers_config_list(self, mock_configuration):
        mock_configuration.params['services']['ci_providers'] = ['ser_1', 'la_3', 'something_4']
...
class TestRepositoryCIFilter(object):
    ...
    def test_filter(self):
        ...
        assert service._filter({'url': 'http://www.another.simple', 'context': 'reject'})
        assert not service._filter({'url': 'http://www.excluded.simple', 'context': 'reject'})
        assert not service._filter({'url': 'http://www.another.reject', 'context': 'simple'})
        assert not service._filter({'url': 'http://www.example.com', 'context': 'nothing'})
        assert not service._filter({'url': 'http://www.example.com', 'context': 'excluded'})
        assert not service._filter({'url': 'http://reject.example.com', 'context': 'ok'})
        assert not service._filter({'url': 'http://www.reject.com', 'context': 'ok'})
        assert not service._filter({'url': 'http://www.reject.com', 'context': 'simple'})
        assert not service._filter({'url': 'http://www.ok.com', 'context': 'simple/reject'})
    ...
    def test_filter_jenkins_excluded(self):
        ...
        assert not service._filter({'url': 'http://www.ok.com', 'context': 'jenkins build'})

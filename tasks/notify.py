...
from database.enums import Decoration, NotificationState, ReportType
...
class NotifyTask(BaseCodecovTask, name=notify_task_name):
    ...
    def run_impl_within_lock(
        ...
        **kwargs,
        ...
        # The NoConfiguredAppsAvailable exception was moved to the notification orchestrator task
        # as once it gets to the notifications it should be cleared of that exception. In theory
        # the RepositoryWithoutValidBotError exception could also be moved
        ...
        if (
            enriched_pull
            # TODO: I believe this should be something we extract to the notification orchestrator
            # but it seems to intertwined with the logic here. The enriched_pull call would need to be
            # cached before moving that to the orchestrator to avoid redundant API calls

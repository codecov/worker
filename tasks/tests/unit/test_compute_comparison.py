...
class TestComputeComparisonTask(object):
    @pytest.mark.asyncio
    async def test_set_state_to_processed(self, dbsession, mocker, mock_repo_provider, mock_storage):
        ...
        data_in_storage = mock_storage.read_file("archive", comparison.report_storage_path)

...
class TestSendEmailTask(object):
    @pytest.mark.asyncio 
    async def test_end_of_trial_email(self, mocker, mock_configuration, dbsession, codecov_vcr, mock_storage, mock_redis):
        ...
        result = await SendEmailTask().run_async(
            ...
            email_type='end-of-trial'
        )
        assert result['job_id'] == '9791f6a7-3d3b-4ae9-8f71-67bd98f33008'

    @pytest.mark.asyncio 
    async def test_send_email_no_owner(self, mocker, mock_configuration, dbsession, codecov_vcr):
        result = await SendEmailTask().run_async(
            db_session=dbsession,
            ownerid=45,
            email_type='end-of-trial'

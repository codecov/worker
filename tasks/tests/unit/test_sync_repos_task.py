...
class TestSyncReposTaskUnit(object):
    ...
    @pytest.mark.asyncio
    async def test_private_repos_set_bot(
        ...
        mocked_1.assert_called_with(
            dbsession, user.ownerid, user.service, expected_ownerids
        )

    @pytest.mark.asyncio
    async def test_set_bot_gitlab_subgroups(
        self, mocker, mock_configuration, dbsession, codecov_vcr
    ):
        ...
        expected_owners_with_bot_set = (
            dbsession.query(Owner.bot_id)
            .filter(
                Owner.service == "gitlab",
                Owner.service_id.in_(
                    ("5542118", "4255344", "4165904", "4570071", "4165905", "5608536")
                ),
            )
            .all()
        )
